...
describe('Test traingle function ',()=>{
    ...
    it('return an error when missed a value',()=>{
        expect(traingleValidity(1,2)).toBe(false)
        expect(traingleValidity(1)).toBe(false)
        expect(traingleValidity(4,8)).toBe(false)
    })
    it('return invalid input ',()=>{
        expect(traingleValidity(2,3,1)).toBe('invalid traingle')
        expect(traingleValidity(2,5,3)).toBe('invalid traingle')
        expect(traingleValidity(9,9,0)).toBe('invalid traingle')
        expect(traingleValidity(0,4,5)).toBe('invalid traingle')
        expect(traingleValidity(8,4,3)).toBe('invalid traingle')
        expect(traingleValidity(0,0,0)).toBe('invalid traingle')
    })
})